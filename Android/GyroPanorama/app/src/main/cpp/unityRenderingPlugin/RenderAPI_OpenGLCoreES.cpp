...
public:
    ...
    void *BeginModifyTexture(void *textureHandle, int textureWidth, int textureHeight,
                             int *outRowPitch) override;

    void EndModifyTexture(void *textureHandle, int textureWidth, int textureHeight, int rowPitch,
                          void *dataPtr) override;

    void *BeginModifyVertexBuffer(void *bufferHandle, size_t *outBufferSize) override;

    void EndModifyVertexBuffer(void *bufferHandle) override;

    void getPixelsFromTexture(void *texture, unsigned char *out, int width, int height) const;
...
void *RenderAPI_OpenGLCoreES::BeginModifyTexture(
        void *textureHandle, int textureWidth, int textureHeight, int *outRowPitch) {
...
void RenderAPI_OpenGLCoreES::EndModifyTexture(
    ...
    if (!dataPtr) {
        LOGE("UnityRendererAPI", "OpenGlEs - EndModifyTexture: null pointer to pixels data");
        return;
    }
    auto gltex = (GLuint) (size_t) (textureHandle);
    glBindTexture(GL_TEXTURE_2D, gltex);
    glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0, textureWidth, textureHeight, GL_RGBA, GL_UNSIGNED_BYTE,
            dataPtr);
}
...
void
RenderAPI_OpenGLCoreES::getPixelsFromTexture(void *textureHandle, unsigned char *out, int width,
                                             int height) const {
    auto gltex = (GLuint) (size_t) (textureHandle);
    glBindTexture(GL_TEXTURE_2D, gltex);
