...
{
    interfaces->Get<IUnityGraphicsVulkan>()->InterceptInitialization(InterceptVulkanInitialization, nullptr);
}
...
static VkPipeline CreateTrianglePipeline(VkDevice device, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, VkPipelineCache pipelineCache)
{
    ...
    if (success)
    {
        ...
        success = vkCreateShaderModule(device, &moduleCreateInfo, nullptr, &shaderStages[0].module) == VK_SUCCESS;
    }
    ...
    if (success)
    {
        ...
        success = vkCreateShaderModule(device, &moduleCreateInfo, nullptr, &shaderStages[1].module) == VK_SUCCESS;
    }
    ...
    if (success)
    {
        ...
        success = vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline) == VK_SUCCESS;
    } 
...
bool RenderAPI_Vulkan::CreateVulkanBuffer(size_t sizeInBytes, VulkanBuffer* buffer, VkBufferUsageFlags usage)
{
    ...
    *buffer = VulkanBuffer();
    ...
    if (vkCreateBuffer(m_Instance.device, &bufferCreateInfo, nullptr, &buffer->buffer) != VK_SUCCESS)
        return false;
    ...
    if (vkAllocateMemory(m_Instance.device, &memoryAllocateInfo, nullptr, &buffer->deviceMemory) != VK_SUCCESS)
    {
        ImmediateDestroyVulkanBuffer(*buffer);
        return false;
    }
...
void RenderAPI_Vulkan::ImmediateDestroyVulkanBuffer(const VulkanBuffer& buffer)
{
    if (buffer.buffer != VK_NULL_HANDLE)
        vkDestroyBuffer(m_Instance.device, buffer.buffer, nullptr);
    ...
    if (buffer.deviceMemory != VK_NULL_HANDLE)
        vkFreeMemory(m_Instance.device, buffer.deviceMemory, nullptr);
}
