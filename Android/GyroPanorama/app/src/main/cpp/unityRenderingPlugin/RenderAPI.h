...
public:
	...
	//
	// Returns pointer into the data buffer to write into (or nullptr on failure), and pitch in bytes of a single texture row.
	...
	// Returns pointer into the data buffer to write into (or nullptr on failure), and buffer size.
	...
	virtual void getPixelsFromTexture(void* texture, unsigned char* out, int width, int height) const;
};
